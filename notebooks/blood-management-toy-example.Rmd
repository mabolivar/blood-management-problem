...
V = {epoch: {b: {i: 20 if epoch < num_epochs - 1 else 0 for i in range(1, 20)} for b in blood_nodes} for epoch in range(num_epochs)}
```

## n-st iteration

```{python}
def update_value_function_deprecated(V_epoch, duals, alpha, delta, monotone):
  ...
  if monotone:
    for node, dual_list in duals.items():
      ...
      for v in dual_list:
        ...
        lower_range = range(max(prev_sloped_unit + 1, num_units - delta), num_units + 1)
        max_value = max(list(V_update.keys()) + [num_units + delta]) + 1
        upper_range = range(num_units + 1, max_value)
        for unit in lower_range:
          prev_V = V_update.get(unit, 0)
          V_update[unit] = max((1-alpha) * prev_V + (alpha) * v["dual"], v_ref)
        
        for unit in upper_range:
          prev_V = V_update.get(unit, v_ref)
          V_update[unit] = min((1-alpha) * prev_V + (alpha) * v["dual"], v_ref)
...
for num_iteration in range(total_iterations):
  ...
  for epoch in range(num_epochs):
    ...
    if epoch > 0:
      ...
      update_value_function_deprecated(V[epoch - 1], duals, alpha, delta, monotone=monotone_enabled)
...
ggplot(value_to_plot) +
  geom_path(aes(x = supply, y = reward),
            size = 1) +
  facet_grid(epoch + node ~ iteration, scales = "fixed") +
