...
num_epocs <- max(as.integer(nodes$epoch)) - 1
supply <- dict()
demand <- dict()
...
for (i in 0:num_epocs){
  ...
  py_set_item(supply, i, py_dict(supply_nodes$node, supply_nodes$b))
...
```{python}
# Get input
inventory = {s: solution["actions"].get((s, ("f", s[0], s[1])), 0) for s in supply}
epoch = 1
supply, demand, allowed_blood_transfers, reward_map = transition_function(
