...
reward = {
    ...
    **{arc: 0 # V[epoch][(arc[0][0], arc[0][1])].get(arc[0][2], 0)
  ...
  solution = {
      ...
      'duals': {
          node: {'dual': mip_solver.LookupConstraint(str(node)).DualValue(),
                 "supply": b}
          for node, b in supply.items() if node[0] =="s"
      }
  }
  return solution
...
```{python}
solve_myopic_network(nodes, b, arcs, upper, reward)
```
```{python}
base_solution = solve_myopic_network(nodes, b, arcs, upper, reward)
print(base_solution)
duals = dict()
for k, v in b.items():
  if k[0] == "s" and k != "sink":
    b[k]= v + 1
    b['sink']-= 1
    sol = solve_myopic_network(nodes, b, arcs, upper, reward)
    print(k, b)
    print(sol)
